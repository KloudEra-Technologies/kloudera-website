...
export default function PartnersPage() {
  ...
  const filteredFeatured = FEATURED_PARTNERS.filter(p => 
    p.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
    p.tagline.toLowerCase().includes(searchQuery.toLowerCase()) ||
    p.details.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const filteredAlliances = OTHER_ALLIANCES.filter(name => 
    ...
    <div className="min-h-screen bg-black text-zinc-100 flex flex-col font-sans selection:bg-teal-500/30">
      ...
      <header className="border-b border-teal-500/20 bg-zinc-950/60 p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center backdrop-blur-md">
        <div className="font-mono">
          <span className="text-[10px] font-bold text-teal-400 tracking-widest uppercase">KLOUDERA TECHNOLOGIES // GLOBAL ECOSYSTEM</span>
      ...
      <main className="flex-1 p-6 max-w-5xl mx-auto w-full space-y-16 py-16 font-mono text-xs">
        ...
        <div className="text-center space-y-4 max-w-xl mx-auto">
          ...
          <h2 className="text-lg font-bold text-white uppercase mt-2">Enterprise Alliances & Security Nodes</h2>
          <p className="text-zinc-500 text-[10.5px] leading-relaxed">
            We integrate with top-tier cybersecurity networks, cloud providers, and hardware infrastructure suppliers to deliver threat-shielded operational stability.
          </p>
        </div>

        {/* Featured Partners Section */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredFeatured.map((partner, idx) => (
            <div 
              key={idx}
